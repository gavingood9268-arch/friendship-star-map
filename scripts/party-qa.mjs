...
await act(host, { type: 'add-question', prompt: '谁最可能临时买票去旅行？' });
const sockets = players.slice(1).map((player) => new WebSocket(`ws://127.0.0.1:4173/ws?room=${code}&participant=${player.participantId}&token=${player.token}`));
